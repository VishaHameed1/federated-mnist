...
def load_data():
    ...
    data_dir = "./data"
    dataset_url = config.KAGGLE_DATASET_URL
    ...
    if config.KAGGLE_USERNAME and config.KAGGLE_KEY:
        ...
        with open("kaggle.json", "w") as f:
            ...
            json.dump({"username": config.KAGGLE_USERNAME, "key": config.KAGGL_KEY}, f)
    ...
    if config.DATASET_TYPE == "mnist":
        ...
        X = df.drop(columns=[config.CSV_TARGET_COLUMN])
        y = df[config.CSV_TARGET_COLUMN]
